...
$(document).ready(function () {
  ...
  // if($('#galleryWrapper').length){
    ...
    var $cartitems = JSON.parse(sessionStorage.getItem('cartItems')) || [],
        ...
        $cartTotal = 0;
    var $delivery_option = $('option:selected', '.delivery_option').val();
    ...
    function removeProduct1(id, that){
        var check;
        for(var i = 0; i < $cartquantity; i++){
            if(id === $cartitems[i].id){
                $cartitems.splice(i, 1);
        ...
        that.parent('div').slideToggle().remove();
        getCartLength();
        updateTotal();
        updateSession();
        if($cartquantity === 0){
    ...
    function updateTotal(){
        ...
        if($('#carttotal').length){
            ...
            // if ($delivery != 0) {
              $cartTotal += Number($delivery_option);

              $('#carttotal').html('$ ' + $cartTotal);
              var pay_url = base_url+'shop/pay?product_price='+$cartTotal+'&product_id='+ $cartitems[0].id +'&product_name='+$cartitems[0].name;
              $('.pay_btn').attr('href', pay_url);  
            // }
            console.log(pay_url);
            console.log($delivery_option);
        }
    ...
    $('.delivery_option').change(function(){
      $delivery_option = $('option:selected', this).val();
      updateTotal();
    });
        ...
        if(type === 1){
            return '<div class="clearfix">' + 
                        ...
                        '<div class="big-content small-content">' + 
                          '<h5>$ ' + price + '</h5>' + 
                        '</div>' + 
                        '<a href="#" class="removeProduct1 btn btn-warning btn-small" data-id="' + id + '">' + 
                          ' Remove ' + 
                        '</a>' +
                    '</div>';
        }
